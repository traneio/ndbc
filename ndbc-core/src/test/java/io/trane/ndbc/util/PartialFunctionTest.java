...
public class PartialFunctionTest {
	...
	public void when() {
		...
		final PartialFunction<TestInterface, Integer> pf = PartialFunction.when(TestClass1.class, v -> {
	...
	public void whenMismatch() {
		...
		final TestInterface value = new TestClass1();
		final PartialFunction<TestInterface, Integer> pf = PartialFunction.when(TestClass2.class, v -> {
			assertEquals(v, value);
			return 2;
		});
	...
	public void orElsePredicate() {
		final TestInterface value = new TestClass1();
		final Integer result = 2;
		final PartialFunction<TestInterface, Integer> pf = PartialFunction.<TestInterface, Integer>apply()
				.orElse(v -> v == value, v -> result);
		assertEquals(result, pf.applyOrElse(value, this::notExpected));
	}
	...
	public void lift() {
		...
		final PartialFunction<TestInterface, Integer> pf = PartialFunction.when(TestClass1.class, v -> result);
		assertEquals(Optional.of(result), pf.lift().apply(new TestClass1()));
	}

	@Test
	public void liftMismatch() {
		final PartialFunction<TestInterface, Integer> pf = PartialFunction.when(TestClass2.class, v -> 33);
