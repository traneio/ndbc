...
public final class EncodingRegistry {
  ...
  public EncodingRegistry(final Optional<List<Encoding<?, ?>>> customEncodings, Charset charset) {
    ...
    List<Encoding<?, ?>> defaultEncodings = Arrays.asList(new BigDecimalEncoding(charset),
        ...
        new IntegerEncoding(charset), new LocalDateEncoding(charset), new LocalDateTimeEncoding(charset),
        new LocalTimeEncoding(charset),
        new LongEncoding(charset), new UUIDEncoding(charset), new OffsetTimeEncoding(charset),
        new ByteEncoding(charset), new ShortEncoding(charset),
        new StringEncoding(charset), new BigDecimalArrayEncoding(new BigDecimalEncoding(charset), charset),
