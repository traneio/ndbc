...
public final class EncodingRegistry {
	...
	private static final List<Encoding<?, ?>> defaultEncodings = Arrays.asList(new BigDecimalEncoding(),
			...
			new IntegerEncoding(), new LocalDateEncoding(), new LocalDateTimeEncoding(), new LocalTimeEncoding(),
			new LongEncoding(), new UUIDEncoding(), new OffsetTimeEncoding(), new ByteEncoding(), new ShortEncoding(),
			new StringEncoding(), new BigDecimalArrayEncoding(new BigDecimalEncoding()),
			new BooleanArrayEncoding(new BooleanEncoding()), new ByteArrayArrayEncoding(new ByteArrayEncoding()),
			new DoubleArrayEncoding(new DoubleEncoding()), new FloatArrayEncoding(new FloatEncoding()),
			new IntegerArrayEncoding(new IntegerEncoding()), new LocalDateArrayEncoding(new LocalDateEncoding()),
			new LocalDateTimeArrayEncoding(new LocalDateTimeEncoding()),
