...
public final class QueryResultExchange {
  ...
  public QueryResultExchange(final EncodingRegistry encoding, Unmarshallers unmarshallers) {
  ...
  private final Exchange<List<DataRow>> gatherDataRows(final List<DataRow> rows) {
    return Exchange
        .receive(
            unmarshallers.emptyQueryResponse
                .orElse(unmarshallers.commandComplete)
                .orElse(unmarshallers.dataRow))
        .flatMap(msg -> {
