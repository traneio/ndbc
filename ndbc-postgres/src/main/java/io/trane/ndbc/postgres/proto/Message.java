...
public interface Message {
	...
	public static final class Bind implements ClientMessage {
		 ...
		 * the number of parameters needed by the query. Next, the following pair of
		 * fields appear for each parameter: The value of the parameter, in the format
		 * indicated by the associated format code. n is the above length.
		 */
		public final List<Value<?>> fields;

		/**
		 * The number of parameter format codes that follow (denoted C below). This can
		 * be zero to indicate that there are no parameters or that the parameters all
		 * use the default format (text); or one, in which case the specified format
	...
	public static abstract class CommandComplete implements ServerMessage {
		 ...
		 * of rows inserted. oid is the object ID of the inserted row if rows is 1 and
		...
		/**
		 * For a MOVE command, the tag is MOVE rows where rows is the number of rows the
		...
		/**
		 * For a SELECT or CREATE TABLE AS command, the tag is SELECT rows where rows is
	...
	public static final class FunctionCall implements ClientMessage {
		 ...
		 * No value bytes follow in the NULL case. The value of the argument, in the
		 * format indicated by the associated format code. n is the above length.
		 */
		public final byte[][] fields;

		/**
		 * The number of argument format codes that follow (denoted C below). This can
		 * be zero to indicate that there are no arguments or that the arguments all use
		 * the default format (text); or one, in which case the specified format code is
		 * applied to all arguments; or it can equal the actual number of arguments. The
		 * argument format codes. Each must presently be zero (text) or one (binary).
		 */
	...
	public static final class FunctionCallResponse implements ServerMessage {
		 ...
		 * include itself). Can be zero. As a special case, -1 indicates a NULL function
	...
	public static final class NotificationResponse implements ServerMessage {
		...
		@Override
		public String toString() {
			return "NotificationResponse [channelName=" + channelName + ", payload=" + payload + ", processID="
					+ processID + "]";
		}
	}
	...
	public static final class ParameterDescription implements ServerMessage {
		/**
		 * Then, for each parameter, there is the following: Specifies the object ID of
	...
	public static final class Parse implements ClientMessage {
		 ...
		 * Then, for each parameter, there is the following: Specifies the object ID of
		 * the parameter data type. Placing a zero here is equivalent to leaving the
		 * type unspecified.
		 */
		...
		@Override
		public String toString() {
			return "Parse [destinationName=" + destinationName + ", params=" + params + ", query=" + query + "]";
		}
	}
	...
	public static final class RowDescription implements ServerMessage {
		public static final class Field {
			...
			@Override
			public String toString() {
				return "Field [attributeNumber=" + attributeNumber + ", dataType=" + dataType + ", dataTypeSize="
						+ dataTypeSize + ", formatCode=" + formatCode + ", name=" + name + ", objectID=" + objectID
	...
	public static final class StartupMessage implements ClientMessage {
		 ...
		 * last name/value pair. Parameters can appear in any order. user is required,
