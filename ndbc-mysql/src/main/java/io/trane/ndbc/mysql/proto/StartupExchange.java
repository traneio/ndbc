...
public class StartupExchange {
	...
	public Exchange<String> apply(final String username, final Optional<String> password,
		...
		return Exchange
				.receive(PartialFunction.when(InitialHandshakeMessage.class,
						doHandshake(username, password, database, encoding)))
	...
	private final PartialFunction<ServerMessage, Exchange<Void>> okResponse = PartialFunction
			.when(OkResponseMessage.class, msg -> Exchange.VOID);
