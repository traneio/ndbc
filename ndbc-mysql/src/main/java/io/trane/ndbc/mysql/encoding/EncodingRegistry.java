...
public final class EncodingRegistry {
  ...
  public EncodingRegistry(final Optional<List<Encoding<?, ?>>> customEncodings, final Charset charset) {
    ...
    final List<Encoding<?, ?>> defaultEncodings = Arrays.asList(new StringEncoding(charset), new LongEncoding(charset));
  ...
  private final <T> Encoding<T, Value<T>> resolve(final int fieldType) {
    ...
    if ((enc = (Encoding<T, Value<T>>) byOid.get(fieldType)) != null)
      ...
      throw new NdbcException("Can't decode value of type " + fieldType);
  }
