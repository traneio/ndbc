...
public class ExecuteStatementCommandMarshaller implements Marshaller<ExecuteStatementCommand> {
  ...
  public ExecuteStatementCommandMarshaller(EncodingRegistry encoding) {
  ...
  public void apply(final ExecuteStatementCommand command, final BufferWriter bw) {
    ...
    packet.writeByte((byte) 0x01); // CURSOR_TYPE_READ_ONLY
  ...
  private void writeNullBitmap(final PacketBufferWriter packet, List<Value<?>> values) {
    byte[] bitmap = new byte[(values.size() + 7) / 8];
  ...
  private void writeParameterTypes(final PacketBufferWriter packet, List<Value<?>> values) {
    for (Value<?> value : values)
      packet.writeUnsignedShort(encoding.fieldType(value));
  }

  private void writeValues(final PacketBufferWriter packet, List<Value<?>> values) {
    for (Value<?> value : values)
