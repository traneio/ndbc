...
public class HandshakeTest {
	...
	private final Logger logger = Logger.getLogger(HandshakeTest.class.getName());
	...
	public void handShakeAndQuery() throws IOException {
		...
		final BufferWriter bw = new TestSyncBufferWriter(clientSocket.getOutputStream(), utf8);
		final BufferReader br = new TestSyncBufferReader(clientSocket.getInputStream(), utf8);

		// StartupExchange
		final InitialHandshakeMessage serverHandshake = initialHandshakePacketUnmarshaller.decode(br);
		info("RECEIVED:" + serverHandshake);

		final HandshakeResponseMessage handshakeResponsePacketMessage = new HandshakeResponseMessage(
	...
	public void handshakeResponsePacket() {
		final byte[] salt = {70, 38, 43, 66, 74, 48, 79, 126, 76, 66, 70, 118, 67, 40, 63, 68, 120, 80, 103, 54};
		final HandshakeResponseMessage handshakeResponsePacketMessage = new HandshakeResponseMessage(1, "username",
		...
		final byte[] expectedHashedPassword = HandshakeResponsePacketMarshaller.scramble411("password", salt, utf8);
